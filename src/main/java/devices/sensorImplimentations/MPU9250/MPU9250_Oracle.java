...
public class MPU9250_Oracle implements Accelerometer, Gyroscope, Magnetometer, Thermometer, Runnable
{
    ...
    private void initAK8963() throws InterruptedException, IOException
    {
        ...
        write(Device.ak8963, Registers.AK8963_CNTL,(byte) 0x00); // Power down magnetometer
        Thread.sleep(10);
        write(Device.ak8963, Registers.AK8963_CNTL, (byte)0x0F); // Enter Fuse ROM access mode
        Thread.sleep(10);
        rawData = read(Device.ak8963, Registers.AK8963_ASAX,3);  // Read the x-, y-, and z-axis calibration values
        ...
        write(Device.ak8963, Registers.AK8963_CNTL, (byte)(magScale.getValue() << 4 | magMode.getMode())); // Set magnetometer data resolution and sample ODR
        Thread.sleep(10);
    }
    ...
    private void initMPU9250() throws IOException, InterruptedException
    {
        ...
        write(Device.mpu9250, Registers.PWR_MGMT_1, (byte)0x00);
        Thread.sleep(100); // Wait for all registers to reset

        // get stable time source
        write(Device.mpu9250, Registers.PWR_MGMT_1, (byte)0x01);  // Auto select clock source to be PLL gyroscope reference if ready else
        Thread.sleep(200);
        ...
        write(Device.mpu9250, Registers.CONFIG, (byte)0x03);

        // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
        write(Device.mpu9250, Registers.SMPLRT_DIV, (byte)0x04);  // Use a 200 Hz rate; a rate consistent with the filter update rate
        // determined inset in CONFIG above

        // Set gyroscope full scale range
        // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
        byte c = read(Device.mpu9250, Registers.GYRO_CONFIG); // get current GYRO_CONFIG register value
        ...
        write(Device.mpu9250, Registers.GYRO_CONFIG, c ); // Write new GYRO_CONFIG value to register

        // Set accelerometer full-scale range configuration
        c = read(Device.mpu9250, Registers.ACCEL_CONFIG); // get current ACCEL_CONFIG register value
        // c = c & ~0xE0; // Clear self-test bits [7:5]
        c = (byte)(c & ~0x18);  // Clear AFS bits [4:3]
        c = (byte)(c | accScale.getValue() << 3); // Set full scale range for the accelerometer
        write(Device.mpu9250, Registers.ACCEL_CONFIG, c); // Write new ACCEL_CONFIG register value

        // Set accelerometer sample rate configuration
        // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
        // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
        c = read(Device.mpu9250, Registers.ACCEL_CONFIG2.getAddress()); // get current ACCEL_CONFIG2 register value
        ...
        write(Device.mpu9250, Registers.ACCEL_CONFIG2, c); // Write new ACCEL_CONFIG2 register value
        ...
        write(Device.mpu9250, Registers.INT_PIN_CFG, (byte)0x12);  // INT is 50 microsecond pulse and any read to clear
        write(Device.mpu9250, Registers.INT_ENABLE, (byte)0x01);  // Enable data ready (bit 0) interrupt
        Thread.sleep(100);
    }
    ...
    private void calibrateGyroAcc() throws IOException, InterruptedException
    {
        ...
        write(Device.mpu9250, Registers.PWR_MGMT_1,(byte)0x80);
        Thread.sleep(100);

        // get stable time source; Auto select clock source to be PLL gyroscope reference if ready
        // else use the internal oscillator, bits 2:0 = 001
        write(Device.mpu9250, Registers.PWR_MGMT_1,(byte)0x01);
        write(Device.mpu9250, Registers.PWR_MGMT_2,(byte)0x00);
        Thread.sleep(200);


        // Configure device for bias calculation
        write(Device.mpu9250, Registers.INT_ENABLE,(byte) 0x00);   // Disable all interrupts
        write(Device.mpu9250, Registers.FIFO_EN,(byte) 0x00);      // Disable FIFO
        write(Device.mpu9250, Registers.PWR_MGMT_1,(byte) 0x00);   // Turn on internal clock source
        write(Device.mpu9250, Registers.I2C_MST_CTRL,(byte) 0x00); // Disable I2C master
        write(Device.mpu9250, Registers.USER_CTRL,(byte) 0x00);    // Disable FIFO and I2C master modes
        write(Device.mpu9250, Registers.USER_CTRL,(byte) 0x0C);    // Reset FIFO and DMP
        Thread.sleep(15);

        // Configure MPU6050 gyro and accelerometer for bias calculation
        write(Device.mpu9250, Registers.CONFIG,(byte) 0x01);       // Set low-pass filter to 188 Hz
        write(Device.mpu9250, Registers.SMPLRT_DIV,(byte) 0x00);   // Set sample rate to 1 kHz
        write(Device.mpu9250, Registers.GYRO_CONFIG,(byte) 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
        write(Device.mpu9250, Registers.ACCEL_CONFIG,(byte) 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity

        int gyrosensitivity = 131;     // = 131 LSB/degrees/sec
        int accelSensitivity = 16384;  // = 16384 LSB/g

        // Configure FIFO to capture accelerometer and gyro data for bias calculation
        write(Device.mpu9250, Registers.USER_CTRL, 0x40);   // Enable FIFO
        write(Device.mpu9250, Registers.FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO  (max size 512 bytes in MPU-9150)
        Thread.sleep(40); // accumulate 40 samples in 40 milliseconds = 480 bytes

        // At end of sample accumulation, turn off FIFO sensor read
        write(Device.mpu9250, Registers.FIFO_EN,0x00);        // Disable gyro and accelerometer sensors for FIFO
        byte[] buffer = new byte[2];
        buffer = read(Device.mpu9250, Registers.FIFO_COUNTH,2); // read FIFO sample count
        ...
        {
            buffer = read(Device.mpu9250, Registers.FIFO_R_W,12); // read FIFO sample count
        ...
        write(Device.mpu9250, Registers.XG_OFFSET_H, buffer[0]);
        write(Device.mpu9250, Registers.XG_OFFSET_L, buffer[1]);
        write(Device.mpu9250, Registers.YG_OFFSET_H, buffer[2]);
        write(Device.mpu9250, Registers.YG_OFFSET_L, buffer[3]);
        write(Device.mpu9250, Registers.ZG_OFFSET_H, buffer[4]);
        write(Device.mpu9250, Registers.ZG_OFFSET_L, buffer[5]);

        int[] accelBiasReg = new int[]{0,0,0};
        buffer = read(Device.mpu9250, Registers.XA_OFFSET_H,2);
        accelBiasReg[0] = (buffer[0] << 8) | buffer[1];
        buffer = read(Device.mpu9250, Registers.YA_OFFSET_H,2);
        accelBiasReg[1] = (buffer[0] << 8) | buffer[1];
        buffer = read(Device.mpu9250, Registers.ZA_OFFSET_H,2);
    ...
    private void selfTest() throws IOException, InterruptedException
    {
        ...
        write(Device.mpu9250, Registers.SMPLRT_DIV,0x00);
        // Set gyro sample rate to 1 kHz and DLPF to 92 Hz
        write(Device.mpu9250, Registers.CONFIG,0x02);
        // Set full scale range for the gyro to 250 dps
        write(Device.mpu9250, Registers.GYRO_CONFIG, GyrScale.GFS_250DPS.getValue());
        // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
        write(Device.mpu9250, Registers.ACCEL_CONFIG2,0x02);
        // Set full scale range for the accelerometer to 2 g
        write(Device.mpu9250, Registers.ACCEL_CONFIG, AccScale.AFS_2G.getValue());
        ...
        {
            buffer = read(Device.mpu9250, Registers.ACCEL_XOUT_H, 6);
            ax += ((buffer[0] << 8) | buffer[1]);
            ay += ((buffer[2] << 8) | buffer[3]);
            az += ((buffer[4] << 8) | buffer[5]);

            buffer = read(Device.mpu9250, Registers.GYRO_XOUT_H,6);
        ...
        write(Device.mpu9250, Registers.ACCEL_CONFIG, 0xE0);
        write(Device.mpu9250, Registers.GYRO_CONFIG, 0xE0);
        Thread.sleep(25);
        ...
        selfTest[3] = read(Device.mpu9250, Registers.SELF_TEST_X_GYRO.getAddress());
        selfTest[4] = read(Device.mpu9250, Registers.SELF_TEST_Y_GYRO.getAddress());
        selfTest[5] = read(Device.mpu9250, Registers.SELF_TEST_Z_GYRO.getAddress());
    ...
    public void updateThermometerData() throws Exception
    {
        ...
        rawData = read(Device.mpu9250, Registers.TEMP_OUT_H,2);  // Read the two raw data registers sequentially into data array
    ...
    {
        int newMagData = (read(Device.ak8963, Registers.AK8963_ST1) & 0x01);
        if (newMagData == 0) return;
        byte[] buffer = new byte[7];
        buffer =read(Device.ak8963, Registers.AK8963_ST1,7);
    ...
    public void updateGyroscopeData() throws IOException
    {
        ...
        rawData = read(Device.mpu9250, Registers.GYRO_XOUT_H,6);  // Read the six raw data registers sequentially into data array
    ...
    public void updateAccelerometerData() throws IOException
    {
        float x,y,z;
        byte rawData[] = new byte[6];  // x/y/z gyro register data stored here
        rawData = read(Device.mpu9250, Registers.ACCEL_XOUT_H, 6);  // Read the six raw data registers sequentially into data array
        x = (rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
        y = (rawData[2] << 8) | rawData[3] ;
        z = (rawData[4] << 8) | rawData[5] ;
    ...
    private void write(Device device, Registers register, int value)
    {
    ...
    private void write(Device deviceType, Registers register, byte value)
    {
        ...
        try {
            ...
            buffer.put(value);
            device.write(register.getAddress(),1,buffer);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            // Close device
    ...
    private byte read(Device device, Registers register)
    {
        return read(device,register.getAddress());
    }
    ...
    {
        return read(device,register.getAddress(),length);
    }
